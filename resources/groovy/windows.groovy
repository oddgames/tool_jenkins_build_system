...
def verifyAndroidJdk(String playbackEngines) {
    ...
    if (exists == 'found') {
        ...
        error "[ERROR] Android OpenJDK is not installed and could not be auto-installed.\n" +
              ...
              "  \"Unity Hub.exe\" -- --headless install-modules -v ${env.UNITY_VERSION} -m <jdk-module-id> -cm\n" +
...
"""
            ...
            // Append any teed log files from the artifact path (e.g. play_upload.log)
            ...
            // Write prompt to file to avoid batch escaping issues with newlines
...
'''
            ...
            if (analysisOutput && !analysisOutput.startsWith("Error:")) {
                ...
                try {
                    ...
                    // Build structured result: explanation + errors as separate fields
                    // Format: JSON string that the Slack formatter can parse
                    def result = [
                    ...
                    // Fallback: wrap raw text as explanation-only
